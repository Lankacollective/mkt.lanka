eexport default async function handler(req, res) {
  ...
  try {
    ...
    if (metaData.error) {
      return res.status(200).json({ gastoReal: 0, impresiones: "0", cpm: "$0.00", status: metaData.error.message });
    }

    const data = metaData.data?.[0] || { spend: 0, impressions: 0, cpm: 0 };
    let imp = parseInt(data.impressions || 0);
    let impF = imp >= 1000000 ? (imp/1000000).toFixed(1)+'M' : imp >= 1000 ? (imp/1000).toFixed(1)+'K' : imp.toString();

    res.setHeader('Cache-Control', 's-maxage=3600, stale-while-revalidate');
    return res.status(200).json({ gastoReal: parseFloat(data.spend || 0), impresiones: impF, cpm: `$${parseFloat(data.cpm||0).toFixed(2)}`, status: "Conectado" });

  } catch (e) {
    return res.status(200).json({ gastoReal: 0, impresiones: "0", cpm: "$0.00", status: "Error: " + e.message });
  }
}
